#!/usr/bin/env python3
"""XZ bottle follow for the official camera.
...
def main():
    parser = argparse.ArgumentParser(description='XZ bottle follow for official camera')
    ...
    parser.add_argument('--top-band', type=float, default=0.28)
    parser.add_argument('--bottom-band', type=float, default=0.38)
    parser.add_argument('--edge-margin-px', type=int, default=0)
    parser.add_argument('--log-every', type=int, default=10)
    args = parser.parse_args()

    print('=' * 60)
    print('YOLO XZ follow for official camera')
    ...
    try:
        while True:
            ...
            if target_lift is None:
                target_lift = (args.lift_min + args.lift_max) / 2.0
